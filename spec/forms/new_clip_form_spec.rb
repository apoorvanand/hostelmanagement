...
RSpec.describe NewClipForm do
  # This will create a clip of 2 if no group is passed or a clip of 3
  # if the variable group is passed into the `group:` creator param
  ...
  shared_examples 'success' do
    ...
    it 'creates a clip of correctsize' do
      r = described_class.new(admin: user.admin?, params: p).submit
    ...
    it 'sends emails to non-confirmed groups' do
      ...
      described_class.new(admin: user.admin?, params: p).submit
      expect(StudentMailer).to \
        have_received(:invited_to_clip).exactly(mail_count).times
    end
  end
