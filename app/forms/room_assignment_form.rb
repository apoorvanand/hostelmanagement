...
class RoomAssignmentForm
  ...
  def update_room_assignments
    ActiveRecord::Base.transaction do
      destroy_old_assignments
      create_new_assignments
    end
  end

  def destroy_old_assignments
    params.each do |_student_id, room_id|
      old = RoomAssignment.where(room_id: room_id)
      old.first.destroy! unless old.empty?
    end
  end
